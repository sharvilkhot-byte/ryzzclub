...
interface DilemmaDecisionScreenProps {
  onNext: (decision: string) => void;
  onBack: () => void;
}

export function DilemmaDecisionScreen({ onNext, onBack }: DilemmaDecisionScreenProps) {
  ...
  const options = [
    { id: 'logic-facts', label: 'Use Logic and facts to rationalize', letter: 'A' },
    { id: 'emotions-feelings', label: 'Choose based on emotions and feelings', letter: 'B' },
    { id: 'ask-friends-family', label: 'Ask my friends and family for advice', letter: 'C' },
    { id: 'gpt-it', label: 'GPT it', letter: 'D' }
  ];
      ...
      <div className="flex flex-col justify-center min-h-screen px-4 md:px-8 lg:px-16">
        <div className="max-w-2xl mx-auto w-full md:mx-0 md:ml-8 lg:ml-16">
          ...
          <div className="flex items-center mb-6 md:mb-8">
            ...
            <span className="text-lg md:text-xl" style={{ color: '#5f4cdc' }}>24</span>
          </div>

          {/* Main question */}
          <h1 className="typeform-main-question mb-4 md:mb-6">
            If you're in a dilemma, you would -*
          </h1>

          {/* Subtitle */}
          <p className="typeform-subtext mb-6 md:mb-8" style={{ display: 'inline', boxSizing: 'border-box' }}>
            Choose the first thing you would do
          </p>
              ...
              {options.map((option) => (
                ...
                >
                  <div className="flex items-start gap-3">
                    <span className="w-8 h-8 text-white rounded flex items-center justify-center text-sm font-semibold flex-shrink-0 mt-0.5" style={{ backgroundColor: '#5f4cdc' }}>
                      {option.letter}
                    </span>
                    <span className="typeform-option-text flex-1">{option.label}</span>
                  </div>
                </button>
              ))}
            </div>
