...
interface RestaurantNoticeScreenProps {
  onNext: (notice: string) => void;
  onBack: () => void;
}

export function RestaurantNoticeScreen({ onNext, onBack }: RestaurantNoticeScreenProps) {
  ...
  const options = [
    { id: 'The crowd, the people', label: 'The crowd, the people', letter: 'A' },
    { id: 'The vibe & ambience', label: 'The vibe & ambience', letter: 'B' },
    { id: 'Too hungry to notice', label: 'Too hungry to notice', letter: 'C' },
    { id: "Where's the exit?", label: "Where's the exit?", letter: 'D' }
  ];
      ...
      <div className="flex flex-col justify-center min-h-screen px-4 md:px-8 lg:px-16">
        <div className="max-w-2xl mx-auto w-full md:mx-0 md:ml-8 lg:ml-16">
          ...
          <div className="flex items-center mb-6 md:mb-8">
            ...
            <span className="text-lg md:text-xl" style={{ color: '#5f4cdc' }}>19</span>
          </div>

          {/* Main question */}
          <h1 className="typeform-main-question mb-12 md:mb-16">
            When you walk into a restaurant, what do you first notice?*
          </h1>
